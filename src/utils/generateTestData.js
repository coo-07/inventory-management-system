...
function buildItem(index, stock, threshold) {
...
export function generateTestItems() {
  ...
  // 在庫切れ（3件）: stock === 0
  for (let i = 0; i < 3; i++) {
    items.push(buildItem(index++, 0, 10));
  }

  // 在庫少（5件）: 0 < stock < threshold
  for (let i = 0; i < 5; i++) {
    const threshold = 10;
    items.push(buildItem(index++, 1 + (i % (threshold - 1)), threshold));
  }

  // 在庫あり（12件）: stock >= threshold
  for (let i = 0; i < 12; i++) {
    const threshold = 5 + (i % 10);
    items.push(buildItem(index++, threshold + 5 + (i % 20), threshold));
  }
