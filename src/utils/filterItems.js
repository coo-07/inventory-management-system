...
// 共通関数として切り出している
export function filterItems(items, { search = "", category = "", manufacturer = "", filter = "all" } = {}) {
  return items.filter((item) => {
    const status = getStockStatus(item.stock, item.threshold);
    const matchesSearch = item.name.toLowerCase().includes(search.toLowerCase());
    const matchesCategory = !category || item.category === category;
    const matchesManufacturer = !manufacturer || item.manufacturer === manufacturer;
    const matchesFilter =
      filter === "all" ? true :
      filter === "low" ? status.isLow :
      filter === "out" ? status.isOut :
      filter === "available" ? (!status.isOut && !status.isLow) : true;
    return matchesSearch && matchesCategory && matchesManufacturer && matchesFilter;
  });
}
